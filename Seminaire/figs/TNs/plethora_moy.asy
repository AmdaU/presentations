...
for (int n = 0; n < N; n+=1) {
  psis[n] = makeTensor("$\psi_" + string(n) + "$", (n*dist,0), new Leg[] {legs[n]}, primary, "circle");
...
// label(scale(3.5)*"${\rm moy}($", (0,dist/2 + externalLegLength/2), align=W, fontsize(fontsize));
 label(scale(3.5)*"$\langle$", (0,dist/2 + externalLegLength/2), align=W, fontsize(fontsize));

// label(scale(3.5)*"$)$", (dist*N,dist/2 + externalLegLength/2), align=E, fontsize(fontsize));
 label(scale(3.5)*"$\rangle$", (dist*N,dist/2 + externalLegLength/2), align=E, fontsize(fontsize));
